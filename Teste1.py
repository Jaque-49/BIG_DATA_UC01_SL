...
# Criando o DataFrame Roubo residencia
df_roubo_residencia = df_ocorrencias[['cisp','roubo_residencia']]
df_roubo_residencia = df_roubo_residencia.groupby(['cisp']).sum(['roubo_residencia']).reset_index()

# Criando o DataFrame roubos em comércio e Roubos em residências
df_rc_rs = df_ocorrencias[['cisp','roubo_comercio','roubo_residencia']]
df_rc_rs = df_rc_rs.groupby(['cisp']).sum(['roubo_comercio','roubo_residencia']).reset_index()
...
# Exibindo a base de dados referentes aos roubos em comércio e Roubos em residências
print('\n---- EXIBINDO A BASE referentes aos roubos em comércio e Roubos em residências-----')
print(df_rc_rs.head())
...
# Identificando os outliers superiores e inferiores -  Roubo_residencia
...
# Filtrando o DataFrame -  Roubo_residencia
df_roubo_residencia_outliers_superiores = df_roubo_residencia[df_roubo_residencia['roubo_residencia'] > limite_superior_roubo_residencia]
df_roubo_residencia_outliers_inferiores = df_roubo_residencia[df_roubo_residencia['roubo_residencia'] < limite_inferior_roubo_residencia]

# Obtendo as medidas de dispersão -  Roubo_residencia
...
correl_rs_rc = np.corrcoef(df_rc_rs['roubo_comercio'],df_rc_rs['roubo_residencia'])[0,1]
...
print(f"A correlação entre a recuperação e o Roubo de comércio é {correl_rs_rc:.1f}")
...
# Exibindo os dados -  roubo residencia
print("\n--------- OBTENDO INFORMAÇÕES -  roubo residencia-----------")
print("---------------------------------------------------------------------")
print('------------------ Medidas de Tendência Central ---------------------')
print("---------------------------------------------------------------------")
print(f"A média dos -  roubo residencia é {media_roubo_residencia:.0f}")
print(f"A mediana -  roubo residencia é {mediana_roubo_residencia:.0f}")
print(f"A distância entre a média e a mediana -  roubo residencia é {distancia_roubo_residencia:.2f} %")
print(f"O menor valor dos -  roubo residencia é {minimo_roubo_residencia:.0f}")
print(f"O maior valor -  roubo residencia é {maximo_roubo_residencia:.0f}")
print(f"A amplitude dos valores -  roubo residencia é {amplitude_roubo_residencia:.0f}")
print(f"O valor do q1 - 25% -  roubo residencia é {q1_roubo_residencia:.0f}")
print(f"O valor do q2 - 50% -  roubo residencia é {q2_roubo_residencia:.0f}")
print(f"O valor do q3 - 75% -  roubo residencia é {q3_roubo_residencia:.0f}")
print(f"O valor do iqr = q3 - q1 -  roubo residencia é {iqr_roubo_residencia:.0f}")
print(f"O limite inferior -  roubo residencia é {limite_inferior_roubo_residencia:.0f}")
print(f"O limite superior -  roubo residencia é {limite_superior_roubo_residencia:.0f}")
print(f"A variância -  roubo residencia é {variancia_roubo_residencia:.0f}")
print(f"A distância da variância X média -  roubo residencia é {distancia_var_roubo_residencia:.0f}")
print(f"O desvio padrão -  roubo residencia é {desvio_padrao_roubo_residencia:.0f}")
print(f"O coeficiente de variação -  roubo residencia é {coeficiente_var_roubo_residencia:.0f}")
print('\n- Verificando a existência de outliers inferiores -')
if len(df_roubo_residencia_outliers_inferiores) == 0:
    print("Não existem outliers inferiores")
else:
    print(df_roubo_residencia_outliers_inferiores)
print('\n- Verificando a existência de outliers superiores -')
if len(df_roubo_residencia_outliers_superiores) == 0:
    print("Não existem outliers superiores")
else:
    print(df_roubo_residencia_outliers_superiores)


# Visualizando os dados sobre os roubos de veículos
print('\nVISUALIZANDO OS DADOS...')
plt.subplots(2,3,figsize=(23,8))
plt.suptitle('Análise dos Dados sobre Recuperação e Roubo de comércio',fontsize=16)
...
if controle_superior == 0:
    #não possui out
    df_roubo_comercio_order = df_roubo_comercio.sort_values(by='roubo_comercio',ascending=False)
    df_roubo_maior = df_roubo_comercio_order.head()
    plt.title(' Ranking dos Roubos de comércios')
    plt.barh(df_roubo_maior['cisp'].astype(str),df_roubo_maior['roubo_comercio'])
else:
    #possui out - exibindo dados discrepantes
...
# posição 04: Correlação Roubo de comercio e roubo de residencia
plt.subplot(2,3,4)
plt.title('Comparativo Roubo de comercio e roubo de residencia')
plt.scatter(df_rc_rs['roubo_comercio'],df_rc_rs['roubo_residencia'])
plt.xlabel('roubo_comercio')
plt.ylabel('roubo_residencia')

# posição 04: Correlação Roubo de comercio e roubo de residencia
plt.subplot(2,3,5)
df_roubo_comercio_order = df_roubo_comercio.sort_values(by='roubo_comercio',ascending=True)
df_roubo_menor = df_roubo_comercio_order.head()
plt.title(' Ranking dos Roubos de comércios')
plt.barh(df_roubo_menor['cisp'].astype(str),df_roubo_menor['roubo_comercio'])
